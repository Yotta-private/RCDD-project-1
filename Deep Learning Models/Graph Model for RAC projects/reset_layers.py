...
def reset(model):
    ...
    for layer in model.layers:
        ...
        if '.GraphFP' in str(layer):
            ...
            # Inner weights
            layer.W_inner.set_value((T.tile(W_inner, (layer.depth + 1, 1, 1)).eval() +                 initializers.uniform((layer.depth + 1, layer.inner_dim, layer.inner_dim)).eval()).astype(np.float32))
            layer.b_inner.set_value((T.tile(b_inner, (layer.depth + 1, 1, 1)).eval()  +                 initializers.uniform((layer.depth + 1, 1, layer.inner_dim)).eval()).astype(np.float32))
