...
class Vocabulary(object):
    ...
    def __init__(self, init_from_file=None, max_length=140):
        ...
        self.vocab = dict(zip(self.chars, range(len(self.chars)))) #通过zip将chars和（多少个chars1-15的数字）连接成字典
...
class MolData(Dataset):
    ...
    def __init__(self, fname, voc):
        ...
        with open(fname, 'r') as f:
            for line in f:
                self.smiles.append(line.split()[0]) #一行行的读进smiles列表中

    def __getitem__(self, i): #对某一个smiles进行encoded成一个向量
        mol = self.smiles[i]
...
def tokenize(smiles):
    ...
    '[xx]' as one token."""
    regex = '(\[[^\[\]]{1,10}\])' #匹配到[1-6个不是[]的字符]为一个token
    smiles = replace_halogen(smiles)
    char_list = re.split(regex, smiles)
    tokenized = []
    for char in char_list:
...
def get_smiles_from_file(fname):
    """ Reads a SMILES file and returns a list of filterd origin smiles"""
    with open(fname, 'r') as f:
        smiles_list = []
        for i, line in enumerate(f):
            if i % 10000 == 0:
...
if __name__ == "__main__":
    ...
    smiles_list = canonicalize_smiles_from_file(smiles_file, "./data/ChEMBL_filtered")
    print("Constructing vocabulary...")
    voc_chars = construct_vocabulary(smiles_list, "./data/Voc")
